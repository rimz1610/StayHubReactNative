...
const EventListContent = ({ route, navigation }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get(
        "http://tehreemimran-001-site1.htempurl.com/Event/GetEvents",
        {
          headers: {
      ...
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            ...
            const response = await axios.get(
              `http://tehreemimran-001-site1.htempurl.com/Event/DeleteEvent?Id=${eventId}`,
              {
                headers: {
                  Authorization: `Bearer ${token}`,
                },
  ...
  const renderItem = ({ item }) => (
    <View style={styles.tableRow}>
      ...
      <Text style={styles.tableCell} numberOfLines={1}>
        {moment(item.bookingStartDate).format("MM/DD/YYYY")} -{" "}
        {moment(item.bookingEndDate).format("MM/DD/YYYY")}
      </Text>

      <View style={styles.tableActions}>
        <TouchableOpacity
          onPress={() => navigation.navigate("AddEditEvent", { id: item.id })}
    ...
    <View style={styles.container}>
      ...
      <View style={styles.tableContainer}>
        <View style={styles.tableHeader}>
          ...
          <Text style={styles.tableHeaderText}>Booking</Text>
...
const EventList = () => {
  return (
    ...
    >
      <Drawer.Screen name="EventListContent" component={EventListContent} />
    </Drawer.Navigator>
  );
};

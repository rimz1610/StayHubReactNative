...
const passwordSchema = Yup.object().shape({
    ...
    newPassword: Yup.string().required('Required').min(6, "Password too short").matches(
        /^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])/,
...
const AdminChangePasswordContent = ({ navigation }) => {
    ...
    const handleSubmit = useCallback(async (values, { setSubmitting }) => {
        try {
            ...
            const response = await axios.post("http://majidalipl-001-site5.gtempurl.com/Account/ChangePassword", values, {
                headers: {
...
const AdminChangePassword = ({ navigation }) => {
    ...
    return (
        ...
        >
            <Drawer.Screen name="AdminChangePasswordC" component={AdminChangePasswordContent} />
        </Drawer.Navigator>
    );
};
